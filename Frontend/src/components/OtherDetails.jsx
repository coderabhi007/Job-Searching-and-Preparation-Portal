...
function OtherDetails() {
  const [formData, setFormData] = useState({
    ...
    skills: [
      ...
      "SQL", "HTML", "CSS", "TypeScript", "Django", "Flask", "AWS", "Docker", "Kubernetes"
    ],
    description: "",
    githubProfile: "",
    leetcodeProfile: "",
      ...
      <form onSubmit={handleSubmit} className="p-6 max-w-3xl mx-auto flex flex-col gap-6 bg-white shadow-lg rounded-xl">
        ...
        <div className="space-y-2">
          ...
          {formData.skills.map((skill, idx) => (
            <input key={idx} value={skill} onChange={(e) => handleSkillChange(idx, e.target.value)} className="p-2 border rounded w-full" />
          ))}
          <Button type="button" onClick={handleAddSkill} className="text-sm gap-2 w-fit">
